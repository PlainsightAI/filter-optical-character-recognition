...
# Install pip + filter-optical-character-recognition at version from VERSION file
RUN --mount=type=bind,source=VERSION,target=/tmp/VERSION,ro \
    ...
    pip install --no-cache-dir \
      ...
      "filter-optical-character-recognition==${PKG_VERSION}"

# Create a writable logs dir and hand over /app to appuser
RUN mkdir -p /app/logs && chown -R appuser:appuser /app

USER appuser
CMD ["python", "-m", "filter_optical_character_recognition.filter"]
